...
function statusColor(label) {
  return STATUS_COLORS[label?.toLowerCase()] || STATUS_COLORS.pending
}
...
function reducer(state, action) {
  if (action.type === 'INIT') {
    const atoms = {}
    for (const n of (action.payload.dag?.nodes || [])) {
      atoms[n.id] = n
    }
    return {
      ...state,
      status: action.payload.status,
      dag: action.payload.dag || { nodes: [], edges: [] },
      atoms,
    }
  }
  ...
  switch (event_type) {
    ...
    case 'edge_created': {
      return {
        ...state,
        dag: { ...state.dag, edges: [...state.dag.edges, payload] },
      }
    }
    case 'check_started':
    case 'check_complete': {
    ...
    case 'verdict_emitted': {
      ...
      const updatedAtom = { ...atom, status: payload.verdict?.toLowerCase() }
      return {
        ...state,
...
function AtomNode({ data }) {
  ...
  return (
    <div className={`px-4 py-2 border rounded-md min-w-[150px] shadow-sm bg-[#131720] ${isSelected ? 'border-[#5B5BD6] ring-2 ring-[#5B5BD6]/50' : 'border-white/10'}`}>
...
export default function Review() {
  ...
  const rfEdges = useMemo(() => {
    return edges.map((e, i) => ({
      ...
      source: e.source,
      target: e.target,
      animated: state.atoms[e.source]?.status === 'cascade_risk' || state.atoms[e.target]?.status === 'cascade_risk',
      style: { stroke: state.atoms[e.source]?.status === 'cascade_risk' ? '#ef4444' : 'rgba(255,255,255,0.2)' }
    }))
  }, [edges, state.atoms])
          ...
          {!selected ? (
            ...
            <div className="p-4 space-y-4">
              ...
              {(state.challenges[selected.id] || []).length > 0 && (
                <div className="mt-4">
                  <div className="text-[10px] uppercase tracking-wider opacity-50 mb-2 font-sans">Challenges</div>
                  <ul className="space-y-2">
                    {state.challenges[selected.id].map((c, i) => (
                      <li key={i} className="text-[11px] bg-red-900/20 text-red-200 p-2 rounded font-sans border border-red-900/50">
                        <strong>{c.attacker_agent || 'Attacker'}:</strong> {c.challenge_text || JSON.stringify(c)}
                      </li>
                    ))}
                  </ul>
                </div>
              ...
              {(state.rebuttals[selected.id] || []).length > 0 && (
                <div className="mt-4">
                  <div className="text-[10px] uppercase tracking-wider opacity-50 mb-2 font-sans">Rebuttals</div>
                  <ul className="space-y-2">
                    {state.rebuttals[selected.id].map((r, i) => (
                      <li key={i} className="text-[11px] bg-blue-900/20 text-blue-200 p-2 rounded font-sans border border-blue-900/50">
                        <strong>Defender:</strong> {r.rebuttal_text || JSON.stringify(r)}
                      </li>
                    ))}
                  </ul>
                </div>
