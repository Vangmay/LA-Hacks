...
`FORMALIZATION_LEAN_ENVIRONMENT`. Formalization LLM config intentionally mirrors
the deep-dive thinking profile by default: `FORMALIZATION_MODEL_PROVIDER`,
`FORMALIZATION_MODEL`, `FORMALIZATION_API_KEY_ENV`,
`FORMALIZATION_BASE_URL`, `FORMALIZATION_REASONING_EFFORT`,
`FORMALIZATION_MAX_OUTPUT_TOKENS`, and the `FORMALIZATION_MODEL_*` retry,
